...
private:
	...
	static void PrintChar(const uint8_t c, uint32_t x, uint32_t y,
	                       uint32_t fgcolor, uint32_t bgcolor) __attribute__((nothrow));
	static void PrintCharAlpha(const uint8_t c, uint32_t x, uint32_t y,
	                           uint32_t fgcolor) __attribute__((nothrow));
	...
	static void Fill24(uint32_t left, uint32_t top, uint32_t right,
	                   uint32_t bottom, uint32_t color) __attribute__((nothrow));
	static void Fill32(uint32_t left, uint32_t top, uint32_t right,
	                   uint32_t bottom, uint32_t color) __attribute__((nothrow));
...
public:	
	static ReturnValue Init(const limine_framebuffer_response *lfb_info) __attribute__((nothrow));

	static void SetPixel(uint32_t x, uint32_t y, uint32_t color) __attribute__((nothrow));
	static void Fill(uint32_t left, uint32_t top, uint32_t right,
	                 uint32_t bottom, uint32_t color) __attribute__((nothrow));
	...
	static void SetTitleText(const char *text) __attribute__((nothrow));
	
	static void Print(const char *text) __attribute__((nothrow));
	static void ScrollDown(const uint32_t lines) __attribute__((nothrow));
	...
	static void SetTabSize(uint32_t tab_size) __attribute__((nothrow));
